...
function c101005025.initial_effect(c)
	...
	e5:SetDescription(aux.Stringid(101005025,0))
	e5:SetCategory(CATEGORY_POSITION)
	e5:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e5:SetCode(EVENT_SUMMON_SUCCESS)
	e5:SetTarget(c101005025.postg)
	e5:SetOperation(c101005025.posop)
	c:RegisterEffect(e5)
	local e6=e5:Clone()
	...
	e7:SetDescription(aux.Stringid(101005025,1))
	e7:SetCategory(CATEGORY_CONTROL)
	e7:SetType(EFFECT_TYPE_IGNITION)
	e7:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e7:SetRange(LOCATION_MZONE)
	e7:SetCountLimit(1)
	e7:SetTarget(c101005025.cttg)
	e7:SetOperation(c101005025.ctop)
	c:RegisterEffect(e7)
end
function c101005025.postg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_POSITION,e:GetHandler(),1,0,0)
end
function c101005025.posop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsAttackPos() and c:IsRelateToEffect(e) then
		Duel.ChangePosition(c,POS_FACEUP_DEFENSE)
	end
end
function c101005025.ctfilter1(c,tp)
	local seq=c:GetSequence()
	if seq>4 then return false end
	return (seq>0 and Duel.CheckLocation(tp,LOCATION_MZONE,seq-1))
		or (seq<4 and Duel.CheckLocation(tp,LOCATION_MZONE,seq+1))
end
function c101005025.cttg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c101005025.ctfilter1(chkc,1-tp) end
	if chk==0 then return Duel.IsExistingTarget(c101005025.ctfilter1,tp,0,LOCATION_MZONE,1,nil,1-tp)
		and e:GetHandler():IsControlerCanBeChanged() end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONTROL)
	local g=Duel.SelectTarget(tp,c101005025.ctfilter1,tp,0,LOCATION_MZONE,1,1,nil,1-tp)
...
function c101005025.ctop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	if (seq>0 and Duel.CheckLocation(1-tp,LOCATION_MZONE,seq-1))
		...
		local g1=Duel.GetMatchingGroup(c101005025.ctfilter2,tp,0,LOCATION_MZONE,nil)
