...
function c100238012.initial_effect(c)
	...
	e2:SetCondition(c100238012.tgcon)
	e2:SetTarget(c100238012.tgtg)
	e2:SetOperation(c100238012.tgop)
	c:RegisterEffect(e2) 
	--negate
	...
	e3:SetCondition(c100238012.discon)
	e3:SetCost(c100238012.discost)
	e3:SetTarget(c100238012.distg)
...
function c100238012.value(e,c)
	...
	return Duel.GetMatchingGroupCount(c100238012.atkfilter,tp,LOCATION_GRAVE,0,nil)*300
end
...
function c100238012.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local bc=e:GetHandler():GetBattleTarget()
	local lv=bc:GetLevel()
	e:SetLabel(lv)
	if chk==0 then return Duel.IsExistingMatchingCard(c100238012.tgfilter,tp,LOCATION_DECK,lv,lv,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,1,tp,LOCATION_DECK)
end
function c100238012.tgop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local lv=e:GetLabel()
	local g=Duel.SelectMatchingCard(tp,c100238012.tgfilter,tp,LOCATION_DECK,0,lv,lv,nil)
	if g:GetCount()>0 then
		Duel.SendtoGrave(g,REASON_EFFECT)
	end
end
function c100238012.costfilter(c)
...
function c100238012.discon(e,tp,eg,ep,ev,re,r,rp)
...
function c100238012.distg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_DISABLE,eg,1,0,0)
end
function c100238012.disop(e,tp,eg,ep,ev,re,r,rp)
